...
@implementation SheetController2
...
- (void)algorithmPreferences_Action {
	if (allowEdit) {
		[[ActionController sharedInstance] editAlgorithmPreferencesForKey:myKey preferences:userIDs];
	}
	[self closeSheet];
}
...
- (void)addSubkey_Action {
	[[ActionController sharedInstance] addSubkeyForKey:myKey type:keyType length:length daysToExpire:hasExpirationDate ? getDaysToExpire (expirationDate) : 0];
	[self closeSheet];
}
...
- (void)addUserID_Action {
	[[ActionController sharedInstance] addUserIDForKey:myKey name:name email:email comment:comment];
	[self closeSheet];
}
...
- (void)addSignature:(GPGKey *)key userID:(NSString *)userID {
	...
	NSSet *secKeySet = [[KeychainController sharedInstance] secretKeys];
	...
	GPGKey *aKey;
	NSSet *allKeys = [[KeychainController sharedInstance] allKeys];
	int i = 0;
...
- (void)addSignature_Action {
	[[ActionController sharedInstance] addSignatureForKey:myKey andUserID:myString signKey:[secretKeyFingerprints objectAtIndex:secretKeyId] type:sigType local:localSig daysToExpire:hasExpirationDate ? getDaysToExpire (expirationDate) : 0];
	[self closeSheet];
}
...
- (void)changeExpirationDate_Action {
	[[ActionController sharedInstance] changeExpirationDateForKey:myKey subkey:mySubkey daysToExpire:hasExpirationDate ? getDaysToExpire (expirationDate) : 0];
	[self closeSheet];
}
...
- (void)searchKeys_Action {
	...
	NSArray *keys = [[ActionController sharedInstance] searchKeysWithPattern:pattern];
...
- (void)receiveKeys_Action:(NSSet *)keyIDs {
	NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
	[self performSelectorOnMainThread:@selector(showResultText:) withObject:[[ActionController sharedInstance] receiveKeysWithIDs:keyIDs] waitUntilDone:NO];
	[pool drain];
}
...
- (void)newKey_Action {
	[[ActionController sharedInstance] generateNewKeyWithName:name email:email comment:comment passphrase:((GPG_VERSION == 1) ? passphrase : nil) type:keyType length:length daysToExpire:hasExpirationDate ? getDaysToExpire (expirationDate) : 0];
	[self closeSheet];
}
	...
	if (returnCode == NSOKButton) {
		...
		switch ([[contextInfo objectForKey:@"action"] integerValue]) {
			case GKOpenSavePanelExportKeyAction: {
				...
				NSData *exportData = [[ActionController sharedInstance] exportKeys:keys armored:(exportFormat & 1) allowSecret:allowSecretKeyExport fullExport:NO];
				if (exportData) {
			...
			case GKOpenSavePanelImportKeyAction:
				[[ActionController sharedInstance] importFromURLs:[sheet URLs]];
				break;
			case GKOpenSavePanelAddPhotoAction: {
				GPGKey *key = [contextInfo objectForKey:@"key"];
				NSString *path = [[sheet URL] path];
				[[ActionController sharedInstance] addPhotoForKey:key photoPath:path];
				break; }
			case GKOpenSavePanelSaveRevokeCertificateAction: {
				...
				NSData *exportData = [[ActionController sharedInstance] genRevokeCertificateForKey:key];
				if (exportData) {
					[[NSFileManager defaultManager] createFileAtPath:path contents:exportData attributes:[NSDictionary dictionaryWithObject:[NSNumber numberWithBool:hideExtension] forKey:NSFileExtensionHidden]];
				} else {
